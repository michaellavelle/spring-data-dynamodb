...
public class DynamoDBLocalResource extends ExternalResource {
    ...
    public static <T> CreateTableResult createTable(AmazonDynamoDB ddb, Class<T> domainType) {
        DynamoDBEntityMetadataSupport<T, Serializable> support = new DynamoDBEntityMetadataSupport(domainType);
        DynamoDBEntityInformation<T, Serializable> entityInfo = support.getEntityInformation();

        String tableName = entityInfo.getDynamoDBTableName();
        String hashKeyProperty = entityInfo.getHashKeyPropertyName();
        String hashKeyAttribute = entityInfo.getOverriddenAttributeName(hashKeyProperty);
